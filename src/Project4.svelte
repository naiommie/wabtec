<script lang="ts">
    ...
    async function getCommitData() {
        isLoading = true;
        for (const container of containers) {
            const selectedBranch = selectedValue;
            if (selectedBranch) {
                let page = 1;
...
<div class="mx-8 rounded-lg bg-[rgba(0,0,0,0.2)] p-10 mt-10 text-gray-100">
    ...
    <h2 class="mb-6">This is second version of submodule project that retrieves submodule tags from a Wabtec Gitlab project using the Gitlab API. 
        The main change is in <span class="font-bold text-blue-500">automatic</span> selection of branches. The process of the code can be find 
    ...
    {/if}
    <button on:click={handleRefresh} class="bg-green-700 hover:bg-green-500 text-white font-bold py-2 px-4 rounded mb-4 {isLoading ? 'opacity-50 cursor-not-allowed' : ''}">
        {#if isLoading}
    ...
    <div class="grid grid-cols-4 gap-4 m-4">
        {#each containers as container}
            <div>
                ...
                <select bind:value={selectedValue} on:change={handleDropdownChange} class="text-black rounded-lg p-2 w-full">
            ...
            <table class="w-full mt-8 text-xs">
                <tr>
                    ...
                    {#each submodules as submodule}
                        <th class="text-white p-2 table-auto">{submodule.subname}</th>
                    {/each}
                </tr>
                {#each containers as container}
                    {#if selectedValue}
                        <tr>
                            <td class="text-white p-2">{container.name}</td>
                            {#each commitData[`${container.name}-${selectedValue}`] || [] as commitInfo}
                                <td class="text-white p-2">{commitInfo?.lastPipelineRef || 'N/A'}</td>
                            {/each}
                        </tr>
                    {/if}
                {/each}
